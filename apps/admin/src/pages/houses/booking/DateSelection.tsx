...
export const DateSelection = ({ startDate, setStartDate, endDate, setEndDate }: DateSelectionProps) => {
          ...
          <div className="relative">
            <button 
              ...
              className={`w-full h-11 pl-12 pr-4 rounded-xl border transition-all flex items-center text-left text-sm font-bold shadow-sm
                ${showStartPicker ? 'border-primary ring-4 ring-primary/5 bg-white text-ink' : 'border-line bg-slate-50/50 text-ink hover:border-line-hover'}
              `}
            >
              <CalendarIcon className={`absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 transition-colors ${showStartPicker ? 'text-primary' : 'text-muted'}`} />
        ...
        <div className="w-full md:flex-1 space-y-2 relative" ref={endPickerRef}>
          ...
          <div className="relative">
            <button 
              ...
              className={`w-full h-11 pl-12 pr-4 rounded-xl border transition-all flex items-center text-left text-sm font-bold shadow-sm
                ${!startDate ? 'opacity-50 cursor-not-allowed border-line bg-slate-50/50' : showEndPicker ? 'border-primary ring-4 ring-primary/5 bg-white text-ink' : 'border-line bg-slate-50/50 text-ink hover:border-line-hover'}
              `}
            >
              <CalendarIcon className={`absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 transition-colors ${showEndPicker ? 'text-primary' : 'text-muted'}`} />
