        ...
        <thead className="bg-white/70">
          <tr>
            {columns.map((column) => (
              <th
                key={column.key}
                className="px-5 py-4 text-left text-xs font-bold uppercase tracking-[0.16em] text-tertiary"
              >
                {column.label}
              </th>
            ))}
        ...
        <tbody className="divide-y divide-line">
          {rows.map((row) => (
            <tr key={row.id} className="transition hover:bg-primary-light/70">
              {columns.map((column) => (
                <td key={column.key} className="px-5 py-4 text-sm font-medium text-muted">
