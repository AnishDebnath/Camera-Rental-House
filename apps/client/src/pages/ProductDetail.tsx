...
const ProductDetail = () => {
  ...
  return (
    <>
      ...
      {createPortal(
        <div className="fixed inset-x-0 bottom-6 z-[200] flex justify-center px-4">
          <div className="flex h-16 w-full max-w-[420px] items-center gap-2 rounded-[3rem] bg-ink/90 p-2 shadow-[0_20px_50px_rgba(0,0,0,0.3)] backdrop-blur-xl border border-white/20 transition-all hover:bg-ink lg:max-w-3xl">
            <button
              type="button"
              onClick={() => toggleFavourite(product)}
              className={`flex h-12 w-12 items-center justify-center rounded-full transition-all active:scale-95 ${isFavourite(product.id)
            ...
            <button
              ...
              className={`flex h-12 flex-1 items-center justify-center gap-3 rounded-full px-6 font-bold text-white transition-all active:scale-[0.98] ${product.available_quantity === 0
            ...
            >
              {inCart ? (
                ...
                <>
                  ...
                  <span className="text-[11px] font-black uppercase tracking-widest">Confirm Rental</span>
                </>
              )}
            </button>
          </div>
