...
const Signup = () => {
  ...
  const [files, setFiles] = useState({ aadhaarDoc: null, voterDoc: null });
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <form onSubmit={handleSubmit} className="card-surface space-y-5 p-6">
          {step === 0 ? (
            <>
              {[
                ['fullName', 'Full name', 'Alex Director'],
                ['phone', 'Phone number', '9876543210'],
                ['email', 'Email address', 'alex@example.com'],
          ...
          {step === 1 ? (
            <>
              ...
              {[
                ['aadhaarDoc', 'Aadhaar photocopy', aadhaarRef],
                ['voterDoc', 'Voter card photocopy', voterRef],
              ].map(([key, label, ref]) => (
                <div key={key} className="space-y-2">
                  <span className="text-sm font-medium text-ink">{label}</span>
                  <button type="button" onClick={() => ref.current?.click()} className="flex min-h-36 w-full flex-col items-center justify-center gap-3 rounded-card border border-dashed border-line bg-page text-center">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary-light text-primary">
                      <Upload className="h-5 w-5" />
                    </div>
                    <p className="text-sm font-semibold text-ink">Tap to upload or use camera</p>
                  </button>
                  <input ref={ref} type="file" accept="image/*" capture="environment" className="hidden" onChange={(event) => handleFile(key, event.target.files?.[0])} />
